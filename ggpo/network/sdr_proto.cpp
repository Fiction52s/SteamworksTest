	...
	UdpMsg::connect_status *status)
{
	...
	_sdr->SetListenConnection(p_connection);
...
bool
SdrProtocol::GetEvent(SdrProtocol::Event &e)
{
...
void
SdrProtocol::SendMsg(UdpMsg *msg)
{
	LogMsg("send", msg);
...
void
SdrProtocol::OnMsg(UdpMsg *msg, int len)
{
	bool handled = false;
	typedef bool (SdrProtocol::*DispatchFn)(UdpMsg *msg, int len);
	static const DispatchFn table[] = {
		&SdrProtocol::OnInvalid,             /* Invalid */
		&SdrProtocol::OnSyncRequest,         /* SyncRequest */
		&SdrProtocol::OnSyncReply,           /* SyncReply */
		&SdrProtocol::OnInput,               /* Input */
		&SdrProtocol::OnQualityReport,       /* QualityReport */
		&SdrProtocol::OnQualityReply,        /* QualityReply */
		&SdrProtocol::OnKeepAlive,           /* KeepAlive */
		&SdrProtocol::OnInputAck,            /* InputAck */
	};
...
void
SdrProtocol::UpdateNetworkStats(void)
{
...
void
SdrProtocol::QueueEvent(const SdrProtocol::Event &evt)
{
...
bool
SdrProtocol::GetPeerConnectStatus(int id, int *frame)
{
...
void
SdrProtocol::Log(const char *fmt, ...)
{
	char buf[1024];
	size_t offset;
	va_list args;
...
void
SdrProtocol::LogMsg(const char *prefix, UdpMsg *msg)
{
	switch (msg->hdr.type) {
...
void
SdrProtocol::LogEvent(const char *prefix, const SdrProtocol::Event &evt)
{
	switch (evt.type) {
...
bool
SdrProtocol::OnInvalid(UdpMsg *msg, int len)
{
	ASSERT(FALSE && "Invalid msg in SdrProtocol");
	return false;
}

bool
SdrProtocol::OnSyncRequest(UdpMsg *msg, int len)
{
...
bool
SdrProtocol::OnSyncReply(UdpMsg *msg, int len)
{
	...
	if (--_state.sync.roundtrips_remaining == 0) {
		Log("Synchronized!\n");
		QueueEvent(SdrProtocol::Event(SdrProtocol::Event::Synchronzied));
		_current_state = Running;
		_last_received_input.frame = -1;
		_remote_magic_number = msg->hdr.magic;
	}
	else {
		SdrProtocol::Event evt(SdrProtocol::Event::Synchronizing);
...
bool
SdrProtocol::OnInput(UdpMsg *msg, int len)
{
	/*
		...
		while (offset < numBits) {
			...
			*/
			if (useInputs) {
				...
				*/
				SdrProtocol::Event evt(SdrProtocol::Event::Input);
...
bool
SdrProtocol::OnInputAck(UdpMsg *msg, int len)
{
	/*
	* Get rid of our buffered input
	*/
...
bool
SdrProtocol::OnQualityReport(UdpMsg *msg, int len)
{
...
bool
SdrProtocol::OnQualityReply(UdpMsg *msg, int len)
{
...
bool
SdrProtocol::OnKeepAlive(UdpMsg *msg, int len)
{
	return true;
}

void
SdrProtocol::GetNetworkStats(struct GGPONetworkStats *s)
{
...
void
SdrProtocol::SetLocalFrameNumber(int localFrame)
{
	/*
...
int
SdrProtocol::RecommendFrameDelay()
{
...
void
SdrProtocol::SetDisconnectTimeout(int timeout)
{
	_disconnect_timeout = timeout;
}

void
SdrProtocol::SetDisconnectNotifyStart(int timeout)
{
	_disconnect_notify_start = timeout;
}

void
SdrProtocol::PumpSendQueue()
{
